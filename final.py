"""
Pipeline analítico CafeNorte — respuestas a las 4 preguntas de negocio.

Supuestos principales:
  - CFDI: I=venta, E=nota de crédito (resta), T/P/N no son operaciones de venta.
  ...
  - P1 y P4 solo cubren canal físico: el e-commerce no tiene tienda_id.
  - P1: la ventana la fijan los snapshots (182 días), no las ventas.
    La anualización asume ausencia de estacionalidad.
...
def normalizar_pos(pos, mapeo, costos):
    ...
    if mapeo.sku_pos.duplicated().any():
        raise ValueError("sku_pos duplicado: el merge multiplicaría ventas.")
    ...
    if len(v) != filas:
        raise ValueError("El merge con el mapeo alteró el número de ventas.")

    # Costo vigente por SKU. merge_asof busca hacia atrás la última vigencia;
    # usar el costo actual generaría pérdidas retroactivas que nunca ocurrieron.
...
def normalizar_ecom(ecom, fx):
    ...
    if len(e) != filas:
        raise ValueError("El merge con tipos de cambio duplicó órdenes.")
...
def p1_rotacion(ventas, snaps, nombres):
    ...
    # Stock sumado sobre las 40 tiendas por día, luego promediado sobre los
    # días. min_count=1 evita que un día sin datos cuente como inventario 0.
...
def graficar(mom):
    ...
    ax1.plot(x, mom.fisico / 1e6, "o-", lw=2, color="#2E5E4E", label="Físico")
    ...
    a = .4
    ax2.bar([p - a / 2 for p in pos], mom.fisico_mom, a, color="#2E5E4E", label="Físico")
...
# ─────────────────────────────────────────── modelo analítico (DuckDB)
def persistir(ventas, ecom, d):
    """
    Persiste un modelo dimensional consultable en SQL.

    Conciliación de canales: ambos se unifican en fact_ventas con llave
    común sku_erp. El e-commerce llega por product_handle -> sku_erp vía
    sku_mappings y queda con tienda_id nulo (no atribuible a tienda) y sin
    costo (el catálogo de costos es del ERP, que solo cubre tienda física).
    """
    import duckdb
    ...
    print(f"\n  Órdenes de e-commerce sin sku_erp: {sin_sku:.1%} "
          "(handles fuera del catálogo del ERP)")
    return ruta
...
def main():
    ...
    titulo(f"P1 · TOP 10 SKUs POR ROTACIÓN ({ini1.date()} a {fin1.date()}, {dias1} días)")
    ...
    titulo(f"P2 · QUIEBRES DE >= {MIN_DIAS_QUIEBRE} DÍAS ({ini2.date()} a {fin2.date()})")
    ...
    print(f"\nFísico {tf:,.0f} MXN ({tf / (tf + te):.1%}) | "
          f"E-commerce {te:,.0f} MXN ({te / (tf + te):.1%})")
    print(f"MoM promedio -> físico {mom.fisico_mom.mean():+.1f}% | "
    ...
    titulo("MODELO ANALITICO PERSISTIDO")
